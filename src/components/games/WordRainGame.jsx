...
const WordRainGame = ({ data, title }) => {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto md:px-4 py-0 md:py-8 overflow-hidden touch-none">
      <div className="bg-slate-900 border-x-0 md:border-4 border-slate-800 rounded-none md:rounded-[3rem] shadow-none md:shadow-2xl overflow-hidden relative h-[80vh] md:h-[70vh] md:min-h-[650px] flex flex-col">
        {/* Sky / Header */}
        <div className="bg-slate-800/80 p-3 md:p-6 flex items-center justify-between border-b border-white/10 z-20 backdrop-blur-md">
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <div className="min-w-0">
              <h3 className="text-white font-black uppercase tracking-widest text-[10px] md:text-sm truncate">Hujan Kata</h3>
              <p className="text-sky-400 text-[10px] md:text-xs font-bold truncate">{targetCategory}</p>
            </div>
          </div>

          <div className="flex items-center gap-3 md:gap-6">
            <div className="text-center">
              <p className="text-[8px] md:text-[10px] font-black text-slate-400 uppercase tracking-tighter">Waktu</p>
              <p className={cn("text-base md:text-2xl font-black font-mono leading-none", timeLeft < 10 ? "text-red-500 animate-pulse" : "text-white")}>
                {timeLeft}s
              </p>
            </div>
            <div className="text-center">
              <p className="text-[8px] md:text-[10px] font-black text-slate-400 uppercase tracking-tighter">Skor</p>
              <p className="text-base md:text-2xl font-black text-sky-400 font-mono leading-none">{score}</p>
            </div>
            <div className="flex gap-0.5 md:gap-1">
              {[...Array(3)].map((_, i) => (
                <Heart 
                  key={i} 
                  className={cn("w-4 h-4 md:w-6 md:h-6 transition-all duration-500", i < lives ? "text-rose-500 fill-rose-500 scale-110" : "text-slate-700 scale-90")} 
                />
              ))}
            </div>
          </div>
        ...
        <div className="h-1.5 md:h-2.5 bg-slate-800 relative z-30 shrink-0 shadow-lg">
           <motion.div 
           ...
           <div className="absolute inset-0 flex items-center justify-center">
              <span className="text-[7px] md:text-[9px] font-black text-white uppercase tracking-[0.5em] mix-blend-difference drop-shadow-sm">
        ...
        <div 
          ...
          className="flex-1 relative overflow-hidden bg-[radial-gradient(circle_at_top,_var(--tw-gradient-stops))] from-slate-800 via-slate-900 to-black select-none"
        >
          {/* Decorative Clouds */}
          ...
          {gameState === 'idle' && (
            <div className="absolute inset-0 z-30 flex flex-col items-center justify-center bg-slate-900/80 backdrop-blur-sm p-4 md:p-8 text-center">
              <motion.div
                initial={{ scale: 0.8, opacity: 0 }}
                animate={{ scale: 1, opacity: 1 }}
                className="max-w-md w-full bg-slate-800 border border-white/10 p-6 md:p-10 rounded-3xl md:rounded-[3.5rem] shadow-2xl"
              >
                ...
                <h2 className="text-2xl md:text-4xl font-black text-white mb-3 md:mb-4 uppercase tracking-tighter">Siap Bermain?</h2>
                <p className="text-xs md:text-base text-slate-400 font-bold mb-8 md:mb-10 leading-relaxed">
                  Tangkap kata-kata kategori <span className="text-sky-400">"{targetCategory}"</span> sebelum mereka menghilang!
                </p>
                <button 
                  onClick={startGame}
            ...
            {fallingWords.map((word) => (
              <motion.button
                ...
                className={cn(
                  ...
                  word.isCorrect 
                    ? "bg-sky-500/30 border-sky-400 text-white shadow-sky-500/20" 
                    : "bg-slate-700/30 border-slate-500/50 text-slate-300 shadow-black/20"
                )}
          ...
          {gameState === 'finished' && (
            <div className="absolute inset-0 z-40 bg-slate-900/95 backdrop-blur-xl flex items-center justify-center p-4 md:p-6 text-center">
              <motion.div
                initial={{ y: 50, opacity: 0 }}
                animate={{ y: 0, opacity: 1 }}
                className="max-w-xs md:max-w-sm w-full bg-slate-800 border-2 md:border-4 border-white/10 p-6 md:p-10 rounded-3xl md:rounded-[4rem] shadow-2xl relative"
              >
                ...
                <h2 className="text-2xl md:text-4xl font-black text-white mt-8 md:mt-12 mb-1 md:mb-2 uppercase tracking-tighter">Hebat! 🏆</h2>
                <p className="text-slate-400 font-bold mb-6 md:mb-10 text-[10px] md:text-sm">Petualangan kata selesai!</p>

                <div className="space-y-3 md:space-y-4 mb-6 md:mb-10">
                   <div className="bg-slate-900/80 p-4 md:p-6 rounded-[1.5rem] md:rounded-[2.5rem] border-2 border-white/5 shadow-inner">
                      <p className="text-[8px] md:text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-1">Skor Akhir</p>
                      <p className="text-4xl md:text-6xl font-black text-sky-400 font-mono tracking-tighter tabular-nums drop-shadow-lg leading-none">{score}</p>
                   </div>
                   <div className="flex gap-2 md:gap-4">
                      <div className="flex-1 bg-slate-900/40 p-2 md:p-4 rounded-xl md:rounded-3xl border border-white/5">
                         <p className="text-[7px] md:text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Terbaik</p>
                         <p className="text-base md:text-xl font-black text-white">{maxStreak}</p>
                      </div>
                      <div className="flex-1 bg-slate-900/40 p-2 md:p-4 rounded-xl md:rounded-3xl border border-white/5">
                         <p className="text-[7px] md:text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Level</p>
                         <p className="text-base md:text-xl font-black text-sky-400">{level}</p>
                      </div>
                   </div>
                </div>
                ...
                <button 
                  onClick={startGame}
                  className="w-full py-3 md:py-5 bg-white text-slate-900 rounded-xl md:rounded-[2rem] font-black uppercase tracking-widest hover:bg-sky-500 hover:text-white transition-all shadow-xl active:scale-95 flex items-center justify-center gap-2 md:gap-3 text-xs md:text-base"
                >
