  ...
  return (
    <div className="space-y-6 pb-16">
      ...
      <div className="flex items-center justify-between p-4 bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Manajemen Intro</h1>
          <p className="text-sm text-slate-500 mt-1">Sesuaikan tampilan awal website</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white dark:bg-slate-800 p-6 rounded-lg border border-slate-200 dark:border-slate-700">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-semibold text-slate-800 dark:text-white">Status Intro</h2>
              </div>
              <button
                onClick={() => setConfig(prev => ({ ...prev, intro_active: !prev.intro_active }))}
                className={cn(
                  "flex items-center gap-2 px-3 py-1.5 rounded-lg border transition-colors text-xs font-medium",
                  config.intro_active 
                    ? "bg-teal-100 dark:bg-teal-900/30 border-teal-500 text-teal-700 dark:text-teal-400" 
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-xs font-medium uppercase text-slate-600 dark:text-slate-400">Judul Arab</label>
                  <input 
                    type="text"
                ...
                <div className="space-y-2">
                  <label className="text-xs font-medium uppercase text-slate-600 dark:text-slate-400">Judul Latin</label>
                  <input 
                    type="text"
                    value={config.intro_title_en}
                    onChange={(e) => setConfig(prev => ({ ...prev, intro_title_en: e.target.value }))}
                    className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 p-3 rounded-lg font-semibold"
                  />
                </div>
              </div>
              
              <div className="space-y-2">
                <label className="text-xs font-medium uppercase text-slate-600 dark:text-slate-400">Teks Tombol</label>
                <input 
                  type="text"
          ...
          <div className="bg-white dark:bg-slate-900 p-6 rounded-lg border border-slate-200 dark:border-white/10">
             <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  <Type className="w-5 h-5 text-teal-500" />
                  <h2 className="font-semibold text-slate-800 dark:text-white">Animasi Mengetik</h2>
                </div>
                <button 
                  onClick={addTypingLine}
                  className="p-2 bg-teal-500/10 text-teal-600 rounded-lg hover:bg-teal-500/20 transition-colors"
                >
             ...
             <div className="space-y-3">
                {config.intro_typing_texts.map((text, idx) => (
                  <div 
                    key={idx} 
                    className="flex items-center gap-3 p-2 bg-slate-50 dark:bg-white/5 rounded-lg border border-slate-200 dark:border-white/10 group"
                  >
                    <div className="w-7 h-7 flex items-center justify-center shrink-0 font-medium text-teal-600 text-xs bg-white dark:bg-slate-800 rounded-md">
                      {idx + 1}
                    </div>
                    <input 
                      type="text"
                    ...
                    <button 
                      ...
                      className="p-2 text-slate-300 hover:text-red-500 hover:bg-red-500/10 rounded-md opacity-0 group-hover:opacity-100 transition-all"
                    >
        ...
        <div className="space-y-6">
           <div className="bg-teal-600 text-white p-6 rounded-lg">
              ...
              <h3 className="text-lg font-semibold mb-2 leading-tight">Optimasi Intro</h3>
           ...
           <div className="p-6 bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-white/10">
